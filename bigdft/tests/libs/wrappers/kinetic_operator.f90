...
program kinetic_operator
  ...
  implicit none
  real(f_double) :: crmult,frmult,maxdiff,sigma,maxdiff1,diffc,diffcur
  ...
  integer :: nlr,nspinor,nstress,is,ii,ii_fin,i,iproc,nn,icur,ni
  real(f_double) :: hx,hy,hz,kx,ky,kz,da,tr,hgrid,scal
  ...
  call yaml_argparse(options,&
       ...
       '- {name: sigma, shortname: s, default: 0.7  , help_string: sigma}')
...
!  hgrids=[0.2_f_double,0.2_f_double,0.2_f_double]
  ...
  da=1.d0
  if (nstress.gt.1)  da=hgrids_init(1)/real(nstress-1,kind=8)
...
!  call daub_to_isf(lr,w,tpsi,projector_real)
  ...
  call real_space_gaussian(ncplx_g,n,l,ider,nterm_max,coeff,expo,k,rxyz,oxyz,lr,gaussian)
...
!  call yaml_map('max diff of pr',projector_real(2084897))
  ...
  oxyz=lr%mesh%hgrids*[lr%nsi1,lr%nsi2,lr%nsi3]
  call real_space_laplacian(ncplx_g,n,l,ider,nterm_max,coeff,expo,k,rxyz,oxyz,lr,gaussian)
...
!                  stress_ps(is,4,ii),stress_ps(is,5,ii),stress_ps(is,6,ii)
  ...
  contains
    ...
    subroutine real_space_gaussian(ncplx_g,n,l,ider,nterm_max,coeff,expo,k,rxyz,oxyz,lr,gaussian)
      use gaussians
      use futile
      use locregs
      use box
...
!            call yaml_map('expo',ex)
            funcs(i)=f_function_new(f_gaussian,exponent=ex)
...
!     call yaml_mapping_close()
         ...
         call separable_3d_function(bit,funcs,1.0_f_double,gaussian)
    ...
    subroutine real_space_laplacian(ncplx_g,n,l,ider,nterm_max,coeff,expo,k,rxyz,oxyz,lr,gaussian)
      use gaussians
      use futile
      use locregs
      use box
...
!     call yaml_mapping_close()
         ...
         call separable_3d_laplacian(bit,funcs,-2.0_f_double,gaussian)
      end do !not correctly written, it should be used to define the functions
