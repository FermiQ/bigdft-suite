...
!      use bounds, only: geocode_buffers
      ...
      real(kind=8),dimension(3,ep%nmpl),intent(in),optional :: center_cores
      ...
      real(kind=8),dimension(:,:,:),intent(out),optional :: rho_mp, pot_mp
      ...
      logical, dimension(3) :: peri, perif
      real(kind=8) :: dr
...
!!$          gaussian = f_malloc((/0.to.lmax,is1.to.ie1,is2.to.ie2,is3.to.ie3,1.to.ep%nmpl/),id='gaussian')
      ...
      call calculate_gaussian_1D(is1, ie1, 1, &
           & denspot%dpbox%mesh, shift, ep, gaussians1, nonzero_startend1)
      call calculate_gaussian_1D(is2, ie2, 2, &
           & denspot%dpbox%mesh, shift, ep, gaussians2, nonzero_startend2)
      call calculate_gaussian_1D(is3, ie3, 3, &
           & denspot%dpbox%mesh, shift, ep, gaussians3, nonzero_startend3)
!!$
...
!!$         call f_free(mpz)
      ...
      do impl=1,ep%nmpl
         ...
         rmax(impl) = min((denspot%dpbox%mesh%ndims(1)-31)*0.25d0*denspot%dpbox%mesh%hgrids(1), &
              (denspot%dpbox%mesh%ndims(2)-31)*0.25d0*denspot%dpbox%mesh%hgrids(2), &
              (denspot%dpbox%mesh%ndims(3)-31)*0.25d0*denspot%dpbox%mesh%hgrids(3))
      end do
...
!!$!--- Start old loop -------------------------------------------------------------------------------------
            i3loop: do i3=is3,ie3
               ...
               r(3) = huge(r(3))
               do j3=j3s,j3e
                  dr = real(ii3+j3*denspot%dpbox%mesh%ndims(3),kind=8)*&
                       & denspot%dpbox%mesh%hgrids(3) + shift(3) - ep%mpl(impl)%rxyz(3)
               ...
               i2loop: do i2=is2,ie2
                  ...
                  r(2) = huge(r(2))
                  do j2=j2s,j2e
                     dr = real(ii2+j2*denspot%dpbox%mesh%ndims(2),kind=8)*&
                          & denspot%dpbox%mesh%hgrids(2) + shift(2) - ep%mpl(impl)%rxyz(2)
                  ...
                  i1loop: do i1=is1,ie1
                     ...
                     r(1) = huge(r(1))
                     do j1=j1s,j1e
                        dr = real(ii1+j1*denspot%dpbox%mesh%ndims(1),kind=8)*&
                             & denspot%dpbox%mesh%hgrids(1) + shift(1) - ep%mpl(impl)%rxyz(1)
...
!!$!--- End new iterator loop -------------------------------------------------------------------------------------
      ...
      if ((ie1-is1+1)*(ie2-is2+1)*(ie3-is3+1) > 0) then
         do ithread=0,nthread-1
            ...
            call axpy((ie1-is1+1)*(ie2-is2+1)*(ie3-is3+1), 1.0_gp, potential_loc(is1,is2,is3,ithread), 1, pot(1,1,1,1), 1)
         end do
      end if
...
!!$          call f_free(gaussian)
      ...
      if (ep%nmpl > 0) then
        ...
        call plot_density(0,1,'full_pot.cube',&
             at,at%astruct%rxyz,denspot%pkernel,1,pot)
!!$
...
!      use bounds, only: geocode_buffers
    ...
    subroutine multipole_analysis_driver_new(iproc, nproc, comm, lmax, ixc, smmd, smats, smatm, smatl, &
      ...
      integer :: ilr, impl, mm, lcheck, nelpsp, psp_source, j, lwork, ii, iiat, iilr
      ...
      real(kind=8),dimension(:,:),allocatable :: locregcenter, overlap_small, tmpmat, tempmat, projx
      ...
      real(kind=8),dimension(:,:,:),allocatable :: qmat_tilde, kp
      ...
      integer :: is1, ie1, is2, ie2, is3, ie3, ioffset, icheck, nmax, ist, is, ists, istl
...
!!$      pery=(smmd%geocode == 'P')
!!$      perz=(smmd%geocode /= 'F')
      peri=bc_periodic_dims(geocode_to_bc(smmd%geocode))
      perx=peri(1)
      pery=peri(2)
      perz=peri(3)
      ...
      if (do_ortho==yes) then
          ...
          do ispin=1,smatl%nspin
              ...
              call matrix_matrix_mult_wrapper(iproc, nproc, smatl, &
                   kernel%matrix_compr(ist:), ovrlp_large%matrix_compr(ist:), kernel_ortho(ist:))
          end do
      ...
      projx = f_malloc((/smats%nspin*nmax**2,natpx/),id='projx')
      ...
      !!write(*,*) 'lzd%llr(1)%locregcenter(1:3)', lzd%llr(1)%locregcenter(1:3)
      !!write(*,*) 'smmd%rxyz(1:3,1)', smmd%rxyz(1:3,1)
      !!write(*,*) 'shift',shift
      if (calculate_multipole_matrices) then
          ...
          do ilr=1,lzd%nlr
              ...
              !locregcenter(1:3,ilr) = smmd%rxyz(1:3,iat) -shift(1:3)
              locregcenter(1:3,ilr) = mp_centers(1:3,iat) -shift(1:3)
              !locregcenter(1:3,ilr) = mp_centers(1:3,ilr) -shift(1:3)
              !!write(*,*) 'ilr, iilr, iat, locregcenter(1:3,ilr)',ilr, orbs%inwhichlocreg(ilr), iat, locregcenter(1:3,ilr)
              !iiat = smmd%on_which_atom(orbs%inwhichlocreg(ilr))
              !write(*,*) 'ilr, iiat, locregcenter(1:3,ilr)',ilr, iiat, mp_centers(1:3,iiat) -shift(1:3)
              !!write(*,*) 'lzd%llr(ilr)%locregcenter(1:3)',lzd%llr(ilr)%locregcenter(1:3)
          end do
          ...
          do m=-l,l
              ...
              if (calculate_multipole_matrices) then
                  call calculate_multipole_matrix(iproc, nproc, l, m, nphi, lphi, lphi, nphir, hgrids, &
                   ...
                   !!write(*,*) 'multipole_matrix%matrix_compr(1:smats%nvctr)', &
                   !!            multipole_matrix%matrix_compr(1:smats%nvctr)
                   !!write(*,*) 'multipole_matrix%matrix_compr(smats%nvctr+1:2*smats%nvctr)', &
                   !!            multipole_matrix%matrix_compr(smats%nvctr+1:2*smats%nvctr)
              ...
              do kat=1,natpx
                  kkat = kat + isatx
                  n = nx(kat)
                  qmat_tilde = f_malloc((/n,n,smats%nspin/),id='qmat_tilde')
                  kp = f_malloc((/n,n,smats%nspin/),id='kp')
                  kernel_extracted = f_malloc((/n,n,smats%nspin/),id='kernel_extracted')
                  multipole_extracted = f_malloc((/n,n,smats%nspin/),id='multipole_extracted')
                  ...
                  do ispin=1,smats%nspin
                      is=(ispin-1)*nmax**2+1
                      call calculate_population_matrix(n, kernel_extracted(1,1,ispin), projx(is,kat), &
                           multipole_extracted(1,1,ispin), qmat_tilde(1,1,ispin), kp(1,1,ispin))
                      ...
                      atomic_multipoles(m,l,kkat) = atomic_multipoles(m,l,kkat) + tt
                  end do
      ...
      if (do_check) then
          ...
          test_pot = f_malloc((/size(denspot%V_ext,1),size(denspot%V_ext,2),size(denspot%V_ext,3),1/),id='test_pot')
          ...
          do lcheck=0,lmax
              ...
              call dcopy(size(denspot%V_ext,1)*size(denspot%V_ext,2)*size(denspot%V_ext,3), &
                   denspot%V_ext(1,1,1,1), 1, test_pot(1,1,1,1), 1)
              call potential_from_charge_multipoles(iproc, nproc, at, denspot, ep_check, &
                   shift, verbosity=0, ixc=ixc, pot=test_pot, center_cores=smmd%rxyz, &
              ...
              call compare_charge_and_potential(denspot%dpbox%bitp,&!iproc, is1, ie1, is2, ie2, is3, ie3, &
                   smmd%nat, &
                   rho_exact, rho_mp, pot_exact, pot_mp, denspot%pkernel, at, rxyz, &
  ...
  subroutine extract_matrix(smat, matrix_compr, n, mat_lookup, matrix)
    ...
    ! Local variables
    integer :: icheck, ii, jj, i, j, ind, ispin, ist
...
!!$  end do
  ...
  if (iproc==0) then
      call write_support_functions_multipoles(tmb, atoms, center_locreg, center_orb, lmax, scaled, &
               shift, do_ortho, multipoles)
 ...
 subroutine calculate_gaussian_1D(is, ie, idim, mesh, shift, ep, gaussian_array, nonzero_startend)
   ...
   ! Local variables
   logical :: peri(3), perif(3)
   integer :: i, impl, l, isx, iex, n, imod, nn, nu, nd, js, je, j, nbuf(3)
   ...
   do impl=1,ep%nmpl
       ...
       do i=is,ie
           tt = huge(1.0_gp)
           do j=js,je
              dr = cell_r(mesh, i - nbuf(idim) + j * mesh%ndims(idim), idim) + &
                   & shift(idim) - ep%mpl(impl)%rxyz(idim)
       ...
       !write(*,*) 'B: impl, se', impl, is, ie, nonzero_startend(1,impl), nonzero_startend(2,impl)
   ...
   end subroutine calculate_gaussian_1D
 ...
 subroutine calculate_gaussian_norm(bitp,nproc, is1, ie1, is2, ie2, is3, ie3, ep, &
   ...
   ! Calling arguments
   type(box_iterator), intent(in) :: bitp
   ...
   call f_routine(id='calculate_gaussian_norm')
...
!!$  end function get_quartic_penalty
  ...
  subroutine write_psp_source(ep, psp_source)
    ...
    subroutine compare_charge_and_potential(boxit,nat,& !iproc, is1, ie1, is2, ie2, is3, ie3, nat, &
               rho_exact, rho_mp, pot_exact, pot_mp, kernel, at, rxyz, &
...
!!$    call dscal(ncheck, factor, potential_total(1), 1)
    ...
    subroutine write_support_functions_multipoles(tmb, atoms, center_locreg, center_orb, lmax, scaled, &
               shift, do_ortho, multipoles)
      use yaml_output
      ...
      integer,intent(in) :: lmax
      real(kind=8),dimension(tmb%orbs%norb),intent(in) :: scaled
      real(kind=8),dimension(3),intent(in) :: shift !< global shift of the atomic positions
      character(len=*),intent(in) :: do_ortho
      ...
      call f_routine(id='write_support_functions_multipoles')
