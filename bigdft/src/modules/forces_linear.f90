...
!!$              scalprod_recvbuf, nrecvcounts, nrecvdspls, bigdft_mpi%mpi_comm)
    ...
    subroutine pulay_correction(iproc, nproc, comm, dim_phi, phi, dim_hphi, hphi, &
               smmd, auxl, hgrids, smats, smatm, smatl, overlap, hamiltonian, kernel, &
               orbs_phi, orbs_hphi, collcom_phi, collcom_hphi, lzd_phi, lzd_hphi, fpulay)
      use sparsematrix_base, only: sparse_matrix, matrices, sparse_matrix_metadata, &
                                   sparsematrix_malloc0_ptr, sparsematrix_malloc_ptr, assignment(=), SPARSE_TASKGROUP
      ...
      use sparsematrix_init, only: matrixindex_in_compressed
      use sparsematrix_highlevel, only: matrix_chebyshev_expansion, matrix_matrix_multiplication
      ...
      ! Calling arguments
      integer,intent(in) :: iproc, nproc, comm, dim_phi, dim_hphi
      real(kind=8),dimension(dim_phi),intent(in) :: phi
      real(kind=8),dimension(dim_hphi),intent(in) :: hphi
      type(sparse_matrix_metadata),intent(in) :: smmd
      type(linmat_auxiliary),intent(in) :: auxl
      real(kind=8),dimension(3),intent(in) :: hgrids
      type(sparse_matrix),intent(in) :: smats, smatm, smatl
      type(matrices),intent(in) :: overlap, hamiltonian, kernel
      type(orbitals_data),intent(in) :: orbs_phi, orbs_hphi
      type(comms_linear),intent(in) :: collcom_phi, collcom_hphi
      type(local_zone_descriptors),intent(in) :: lzd_phi, lzd_hphi
      real(kind=8),dimension(3,smmd%nat),intent(out) :: fpulay

      ! Local variables
      integer :: iorb, iiorb, ilr, iilr, iiat, jjorb, jlr, jjlr, jjat, n, ist, i, ind, istxyz
      real(kind=8) :: factor
      type(matrices),dimension(1) :: inv_overlap
      type(matrices) ::  ham_large, tmpmat, prodmat, pulaymat
      real(kind=8),dimension(:),allocatable :: phit_c, phit_f, hphit_c, hphit_f, phi_large, phid, grad
      real(kind=8),dimension(:),allocatable :: gradt_c, gradt_f
      real(kind=8),dimension(:,:),allocatable :: phidxyz

      call f_routine(id='pulay_correction')

      ! Calculate S^-1
      inv_overlap(1)%matrix_compr = sparsematrix_malloc0_ptr(smatl,iaction=SPARSE_TASKGROUP,id='inv_overlap(1)%matrix_comp')
      call matrix_chebyshev_expansion(iproc, nproc, comm, 1, (/-1.d0/), &
           smats, smatl, overlap, inv_overlap)
      write(*,*) 'inv_overlap(1)%matrix_compr',inv_overlap(1)%matrix_compr

      ! Calculate the matrix KHS^-1
      ham_large%matrix_compr = sparsematrix_malloc0_ptr(smatl,iaction=SPARSE_TASKGROUP,id='ham_large%matrix_comp')
      call transform_sparse_matrix(iproc, smatm, smatl, SPARSE_TASKGROUP, 'small_to_large', &
                   smat_in=hamiltonian%matrix_compr, lmat_out=ham_large%matrix_compr)
      tmpmat%matrix_compr = sparsematrix_malloc0_ptr(smatl,iaction=SPARSE_TASKGROUP,id='tmpmat%matrix_comp')
      call matrix_matrix_multiplication(iproc, nproc, smatl, kernel, ham_large, tmpmat)
      prodmat%matrix_compr = sparsematrix_malloc0_ptr(smatl,iaction=SPARSE_TASKGROUP,id='prodmat%matrix_comp')
      call matrix_matrix_multiplication(iproc, nproc, smatl, tmpmat, inv_overlap(1), prodmat)


      ! Transform phi to the large locreg
      phi_large = f_malloc0(dim_hphi,id='phi_large')
      call small_to_large_locreg(iproc, orbs_phi%norb, orbs_phi%norbp, orbs_phi%isorb, orbs_phi%inwhichlocreg, &
           dim_phi, dim_hphi, lzd_phi, lzd_hphi, phi, phi_large)

      ! Tranpose phi and hphi
      phit_c = f_malloc(collcom_hphi%ndimind_c,id='phit_c')
      phit_f = f_malloc(7*collcom_hphi%ndimind_f,id='phit_f')
      call transpose_localized(iproc, nproc, dim_hphi, orbs_hphi, collcom_hphi, &
           TRANSPOSE_FULL, phi_large, phit_c, phit_f, lzd_hphi)
      hphit_c = f_malloc(collcom_hphi%ndimind_c,id='hphit_c')
      hphit_f = f_malloc(7*collcom_hphi%ndimind_f,id='hphit_f')
      call transpose_localized(iproc, nproc, dim_hphi, orbs_hphi, collcom_hphi, &
           TRANSPOSE_FULL, hphi, hphit_c, hphit_f, lzd_hphi)

      ! Build the linear combinations
      gradt_c = f_malloc(collcom_hphi%ndimind_c,id='gradt_c')
      gradt_f = f_malloc(7*collcom_hphi%ndimind_f,id='gradt_f')
      call build_linear_combination_transposed(collcom_hphi, smatl, auxl, prodmat, phit_c, phit_f, &
           .true., gradt_c, gradt_f, iproc)
      call dscal(collcom_hphi%ndimind_c, -1.d0, gradt_c(1), 1)
      call dscal(7*collcom_hphi%ndimind_f, -1.d0, gradt_f(1), 1)
      call build_linear_combination_transposed(collcom_hphi, smatl, auxl, kernel, hphit_c, hphit_f, &
           .false., gradt_c, gradt_f, iproc)
      ...
      factor = 1.d0
      call dscal(collcom_hphi%ndimind_c, factor, gradt_c(1), 1)
      call dscal(7*collcom_hphi%ndimind_f, factor, gradt_f(1), 1)

      ! Untranspose the gradient arrays
      grad = f_malloc(dim_hphi,id='grad')
      call untranspose_localized(iproc, nproc, dim_hphi, orbs_hphi, collcom_hphi, &
           TRANSPOSE_FULL, gradt_c, gradt_f, grad, lzd_hphi)
      write(*,*) 'sum(grad)/factor',sum(grad)/factor

      ! Calculate the derivative of the supoprt functions
      phid = f_malloc(3*dim_hphi,id='phid')
      call get_derivative_supportfunctions(dim_hphi, hgrids(1), lzd_hphi, orbs_hphi, phi_large, phid)

      ! Reorder the derivate basis function
      phidxyz = f_malloc0((/dim_hphi,3/),id='phidxyz')
      ist = 1
      istxyz = 1
      do iorb=1,orbs_hphi%norbp
          iiorb = orbs_hphi%isorb + iorb
          ilr = orbs_hphi%inwhichlocreg(iiorb)
          n = lzd_hphi%llr(ilr)%wfd%nvctr_c + 7*lzd_hphi%llr(ilr)%wfd%nvctr_f
          do i=1,3
              call f_memcpy(src=phid(ist:ist+n-1), dest=phidxyz(istxyz:istxyz+n-1,i))
              write(*,*) 'sum(phid(ist:ist+n-1)), sum(phidxyz(1:n,i))', sum(phid(ist:ist+n-1)), sum(phidxyz(1:n,i))
              ist = ist + n
          end do
          istxyz = istxyz + n
      end do
      write(*,*) 'sum(phid), sum(phidxyz)', sum(phid), sum(phidxyz)
      if (ist /= 3*dim_hphi+1) then
          call f_err_throw(trim(yaml_toa(ist))//'=ist /= 3*dim_hphi+1='//trim(yaml_toa(3*dim_hphi+1)))
      end if

      ! Calculate the overlap matrix for each coordinate
      call f_zero(fpulay)
      pulaymat%matrix_compr = sparsematrix_malloc_ptr(smatl,iaction=SPARSE_TASKGROUP,id='pulaymat%matrix_compr')
      do i=1,3
          call f_zero(pulaymat%matrix_compr)
          call overlap_matrix(grad, dim_hphi, lzd_hphi, orbs_hphi, collcom_hphi, smatl, auxl, pulaymat, phidxyz(1:,i))
          write(*,*) 'pulaymat',pulaymat%matrix_compr
          !call f_zero(fpulay)
          !write(*,*) 'sum(phidxyz(1:,i))', sum(phidxyz(1:,i))
          !call overlap_matrix(phi_large, dim_hphi, lzd_hphi, orbs_hphi, collcom_hphi, smatl, auxl, pulaymat, phidxyz(1:,i))
          !write(*,*) 'pulaymat',pulaymat%matrix_compr
          do iorb=1,orbs_hphi%norbp
              iiorb = orbs_hphi%isorb + iorb
              iilr = orbs_hphi%inwhichlocreg(iiorb)
              iiat = smmd%on_which_atom(iilr)
              do jjorb=1,orbs_hphi%norb
                  jjlr = orbs_hphi%inwhichlocreg(jjorb)
                  jjat = smmd%on_which_atom(jjlr)
                  if (jjat == iiat) then
                      ind = matrixindex_in_compressed(smats, iiat, jjat)
                      fpulay(i,iiat) = fpulay(i,iiat) + pulaymat%matrix_compr(ind)
                  end if
              end do
          end do
      end do
      ...
      ! Check
      call overlap_matrix(hphi, dim_hphi, lzd_hphi, orbs_hphi, collcom_hphi, smatl, auxl, pulaymat, phi_large)
      write(*,*) 'pulaymat%matrix_compr',pulaymat%matrix_compr
      write(*,*) 'hamiltonian%matrix_compr',hamiltonian%matrix_compr
