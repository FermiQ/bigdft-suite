...
!!$  if (geocode == 'F') then
  ...
  if (nproc > 1) then
     ...
     call fmpi_alltoall(sendbuf=psir,&
          sendcounts=ncommarr(0:,1),sdispls=ncommarr(0:,2), &
          recvbuf=psiw,&
          recvcounts=ncommarr(0:,3),rdispls=ncommarr(0:,4),&
...
!!$                   0.d0,.false.,1,quiet='YES')
  ...
  if (nproc > 1) then
     ...
     call fmpi_alltoall(sendbuf=psir,&
          sendcounts=ncommarr(0:,3),sdispls=ncommarr(0:,4), &
          recvbuf=psiw,&
          recvcounts=ncommarr(0:,1),rdispls=ncommarr(0:,2),comm=bigdft_mpi%mpi_comm)
...
!!$  if (lr%geocode == 'F') then
  ...
  if (nproc > 1) then
     ...
     call fmpi_alltoall(sendbuf=psiwocc,sendcounts=ncommocc(0:,1),sdispls=ncommocc(0:,2), &
          recvbuf=psirocc,recvcounts=ncommocc(0:,3),rdispls=ncommocc(0:,4),&
          comm=bigdft_mpi%mpi_comm)
...
!!$  if (geocode == 'F') then
  ...
  if (nproc > 1) then
     ...
     call fmpi_alltoall(sendbuf=psirvirt,sendcounts=ncommvirt(0:,1),sdispls=ncommvirt(0:,2), &
          recvbuf=psiwvirt,recvcounts=ncommvirt(0:,3),rdispls=ncommvirt(0:,4),&
          comm=bigdft_mpi%mpi_comm)
...
!!$                   0.d0,.false.,1,quiet='YES')
  ...
  if (nproc > 1) then
     ...
     call fmpi_alltoall(sendbuf=psirvirt,&
          sendcounts=ncommvirt(0:,3),sdispls=ncommvirt(0:,4), &
          recvbuf=psiwvirt,&
          recvcounts=ncommvirt(0:,1),rdispls=ncommvirt(0:,2),&
          comm=bigdft_mpi%mpi_comm)
...
!!!     end do
!!!
!!!     if (nproc > 1) call fmpi_allreduce(ndatas,FMPI_SUM,comm=bigdft_mpi%mpi_comm)
...
!!!  if (nproc>1) call fmpi_allreduce(eexctX,1,FMPI_SUM,comm=bigdft_mpi%mpi_comm)
!!!
!!!  !the exact exchange energy is half the Hartree energy (which already has another half)
!!!  eexctX=-exctXfac*eexctX
!!!
...
!!!  call OP2P_unitary_test(bigdft_mpi%mpi_comm,iproc,nproc,ngroup,ndim,nvctr_par,symmetric)
!!!
!!!
!!!  !here we can start with the round-robin scheme
!!!  !since the orbitals are all occupied we have to use the symmetric scheme
...
!!$  
!!$  !call MPI_BARRIER(bigdft_mpi%mpi_comm,ierr)
!!$  call fmpi_allreduce(eexctX,1,FMPI_SUM,bigdft_mpi%mpi_comm,ierr)
!!$  
!!$  !the exact exchange energy is half the Hartree energy (which already has another half)
!!$  eexctX=-exctXfac*eexctX
!!$  
...
!!$     end do
!!$
!!$     call fmpi_allreduce(ndatas(1,0,1),2*nproc*op2p%ngroup,FMPI_SUM,bigdft_mpi%mpi_comm,ierr)
!!$     !if(iproc ==0)print *,'iproc,datas',iproc,ndatas
!!$
!!$     do igroup=1,op2p%ngroupp
!!$        if (ndatac(igroup) /=0) then
