...
program PSolver_examples
   ...
   logical :: usegpu
   logical :: lin_PB
   logical, parameter :: old = .false. !.true.
   ...
   real(8), dimension(3) :: hgrids,angdeg,angrad,v,dleps
...
!   type(mpi_environment), intent(in), optional :: mpi_env
   real(kind=8), dimension(:,:,:), allocatable :: eps,potential,pot_ion,rho
   integer :: i1,i2,i3
...
! rho.
     ...
     call test_functions(mesh,geocode,ixc,n01,n02,n03,acell,a_gauss,hx,hy,hz,&
          density,potential,rhopot,pot_ion,iproc,&
          PSol,alpha,beta,gamma) 
...
!   eps=1.d0 
!   if ( trim(PSol)/='VAC') then
!    call PS_gather(pkernel%w%eps,pkernel)
   ! Calculate the charge starting from the potential applying the proper Laplace operator.
!   call ApplyLaplace(mesh,geocode,n01,n02,n03,nspden,hx,hy,hz,potential,rhopot,eps,nord,&
!        mPB,eps0)
!   end if
...
!    pot_ion=0.d0
!    if (iproc==0) then
!     call yaml_mapping_open('Check pkernel%w%eps')
!     call writeroutinePot(n01,n02,n03,nspden,eps,pkernel%max_iter,pot_ion)
!    end if
   ! Calculate the charge starting from the potential applying the proper Laplace operator.
!   call ApplyLaplace(mesh,geocode,n01,n02,n03,nspden,hx,hy,hz,potential,rhopot,eps,nord,&
!        mPB,eps0)
...
!
! if (iproc==0) then
!  call yaml_mapping_open('Comparison between numerical and starting analytical density')
!  call writeroutinePot(n01,n02,n03,1,density,0,rvApp)
!  call yaml_mapping_close()
! end if
