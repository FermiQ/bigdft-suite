...
#       yaml_file (str):
#         You can also pass the yaml input file as generated by the BigDFT
...
#         the split fragments to the system.
#
#         Args:
#           yaml_file (str): The yaml input file as generated by the BigDFT
#             version of openbabel.
#           splitdict (dict): a dictionary whose keys are the names of the new
#             fragments to create, and values are a list of dictionaries that
#             describe the atoms.
#           fragid (str): the id of the fragment to split. If not specified,
#             the first fragment in the fragment list is split.
#         """
...
#             fdict = load(ifile)
#         fdict = fdict["Reading positions"]
#         name = fdict.keys()[0]
#         fdict = fdict[name]
#
#         print(splitdict)
#         print(self.fragments[0])
#         # # Find the fragment to split and pop it
#         # if fragid is None:
#         #     target = self.fragments.pop(0)
#         # else:
...
#         #             break
#         #
#         # for splitid, splitatoms in splitdict.items():
#         #     new_frag = Fragment(units=self.units, id=splitid)
#         #     for sa in splitatoms:
#         #         for ta in targetatoms:
#         #             if norm(array(aj["r"]) - array(ai["r"])) < thresh:
#         #
#         #     self.append(new_frag)
#         # print(target.id)
#
#     def _fill_from_yaml(self, file):
#         """
#         Fill a system from a yaml input file as generated by the BigDFT version
#         of openbabels.
#
#         Args:
#           file (str): the name of the file to build from.
#         """
#         from yaml import load
#
#         # Get at the main part of the file
#         with open(file) as ifile:
#             fdict = load(ifile)
#         fdict = fdict["Reading positions"]
#         name = fdict.keys()[0]
...
#         self._get_units(fdict["units"])
#
#         # Create the fragments in a dictionary, because they might come
#         # in any order.
#         frag_dict = {}
#         for atom in fdict["positions"]:
#             # Determine  the type of atom.
#             # WD: this is why we need to change that input file format.
#             for key in atom.keys():
#                 if key != "frag":
#                     element = key
#                     break
#
#             frag = SetFragId(atom["frag"][0], str(atom["frag"][1]))
#             if not frag in frag_dict:
#                 frag_dict[frag] = Fragment(units=self.units, id=frag)
#
#             frag_dict[frag].append({"sym":element, "r":atom[element]})
#
#         # Now convert the fragment dictionary to a list
...
#         if hasattr(self, 'Q'):
#             dc['global monopole'] = float(self.Q())
#         return dc
#
...
#         labels = []
#         for i, frag in enumerate(self.fragments):
#             pvals.append(frag.purity_indicator)
#             labels.append(frag.id)
#         axs.plot(pvals, 'x--')
#
...
#         """
#         This sorts the fragment list of the system by the id values.
#         """
#         self.fragments.sort(key=lambda x: GetFragTuple(x.id)[1])
#
#     def merge_frag_byid(self, id1, id2):
#         """
#         This routine will merge two fragments based on the given id values.
#
#         Both of these fragments are removed from the system, and replaced
#         by one big fragment.
#
#         Args:
#           id1 (str): the id of the first fragment.
#           id2 (str): the id of the second fragment.
#         """
#         # If the ids are the same, there is no work to do.
#         if id1 == id2:
#             return
#
#         # Find the fragments of interest.
#         for i in range(0, len(self.fragments)):
#             if self.fragments[i].id == id1:
#                 frag1 = self.fragments.pop(i)
#                 break
#         for i in range(0, len(self.fragments)):
#             if self.fragments[i].id == id2:
#                 frag2 = self.fragments.pop(i)
#                 break
#
#         # Merge them together.
#         tuple1 = GetFragTuple(frag1.id)
#         tuple2 = GetFragTuple(frag2.id)
#         merged_num = min(tuple1[1], tuple2[1])
#         mid = SetFragId(tuple1[0]+tuple2[0], str(merged_num))
#         mfrag = Fragment(units=self.units, id=mid)
#         for atom in frag1.atoms + frag2.atoms:
#             mfrag.append(atom)
#
#         # Add to the list
#         self.append(mfrag)
