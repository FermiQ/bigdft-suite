...
class SystemCalculator(Runner):
    ...
    def _get_command(self):
        ...
        if dry_run:
            ...
            command = os.path.join(
                os.environ['BIGDFT_ROOT'], 'bigdft-tool') + ' -a memory-estimation -l'
            if name > 0:
                command += ' --name=' + name
        else:
    ...
    def _ensure_run_directory(self):
        ...
        if ("/" in run_dir or run_dir == ".."):
            raise ValueError(
                "run_dir '%s' where bigdft is executed must be a sub-directory" % run_dir)
    ...
    def _posinp_dictionary_value(self, posinp):
        ...
        if not os.path.isfile(posinp):
            raise ValueError(
                "posinp: The atomic position file '%s' does not exist" % posinp)
        posinpdict = posinp
